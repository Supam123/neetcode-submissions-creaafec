class Solution:
    def subsets(self, nums: List[int]) -> List[List[int]]:
        ...
        return res
        '''
        so we go we add 1 to sub set 2 to subset 3 to subset and then we reach base case
        we append it to res then we after 3 returns cuz  3+1 is 4 we reutn we pop 3 from subset and 
        call dfs(4) again this time we append subset was [1,2] so we append that ti res
        then dfs
        '''
